...
from model import PointNetEncoder, Generator, Discriminator, RewardModel
import itertools
from utils import *
...
name           = f"model/tagmol/{ligand_size}"
log_dir        = f"{name}"
...
opt_enc_gen   = torch.optim.Adam(itertools.chain(encoder.parameters(), generator.parameters(), reward_model.parameters()), lr, (0.0, 0.9))
...
def main():
    ...
    if start_epoch:
        print(f'Loading model checkpoints from {models_dir}...')
        for model in [encoder, generator, discriminator, reward_model]:
            path = os.path.join(models_dir, f'{start_epoch}-{str(type(model)).split(".")[-1][:-2]}.ckpt')
    ...
    for epoch in tqdm(range(start_epoch, max_epoch),  desc='total progress'):
        ...
        for batch, sample_batched in enumerate(train_loader):
            ...
            if (batch+1) % n_critic == 0:
                ...
                _, _, r_properties, f_properties = compute_rdkit_property(r_atoms, r_bonds, f_atoms, f_bonds)

                loss_R = torch.mean((r_pred_properties - r_properties)**2 + \
                                                (f_pred_properties - f_properties)**2)

                loss_G = loss_G_fake + gamma_lr*loss_R
                losses_G.append(loss_G.item())

                loss_G.backward()
                opt_enc_gen.step()

                batch_log += f"loss_g: {loss_G_fake.item():.2f}\t{loss_R.item():.2f}\n"
                ...
                batch_log += f"properties: {torch.mean(r_properties, 0)[0].item():.4f}\t{torch.mean(r_properties, 0)[1].item():.4f}\t{torch.mean(r_properties, 0)[2].item():.2f}\t{torch.mean(f_properties, 0)[0].item():.2f}\t{torch.mean(f_properties, 0)[1].item():.2f}\t{torch.mean(f_properties, 0)[2].item():.2f}\n"
        ...
        if (epoch+1) % save_step == 0:
            for model in [encoder, generator, discriminator, reward_model]:
                path = os.path.join(models_dir, f'{epoch+1}-{str(type(model)).split(".")[-1][:-2]}.ckpt')
